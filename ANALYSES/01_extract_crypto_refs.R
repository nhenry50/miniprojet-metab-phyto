...
read1 <- read.fasta(file = "data/46345_EukRibo_V4_2020-10-27.fas.gz")

# Transform to a table

refs_table <- data.table(id=names(read1),
                         sequence = getSequence(read1,as.string = TRUE) |> unlist(),
                         taxo = getAnnot(read1) |>
                           unlist())
...
seqinr::write.fasta(names=names(refs_table),sequence=refs_table,file.out="outputs/crypto_seq.fasta")

# export taxo

fwrite(refs_table,
       file = "outputs/crypto_seq.tsv",
       sep="\t",
       quote=F)
...
refs_table_derep[,taxoplot:=sub(pattern = "Cryptomonadales\\|(g:)*|g:",replacement="",taxo) |>
